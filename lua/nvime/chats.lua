...
local function dimensions(mode)
	...
	if
		...
		and not (type(cfg.float_width) == "number" and cfg.float_width > 0)
	then
	...
	if
		mode == "dashboard"
		and type(cfg.dashboard_width) == "number"
		and cfg.dashboard_width > 0
		and cfg.dashboard_width <= 1
	then
	...
	if
		mode == "dashboard"
		and type(cfg.dashboard_height) == "number"
		and cfg.dashboard_height > 0
		and cfg.dashboard_height <= 1
	then
...
local function window_config(mode)
	...
	if mode == "dashboard" then
		footer =
			" (1)-(5) tabs | <CR> open | n new | a ask | e edit | R rename | dd delete | r refresh | ? help | q close "
	end
	return {
		relative = "editor",
		width = dim.width,
...
local function open_help()
	...
	if panel.mode == "dashboard" then
		...
		state.panels.chats_help = show_help
				and {
					bufnr = panel.bufnr,
					winid = panel.winid,
					in_buffer = true,
...
local function add_branded_header(lines, marks, subtitle)
	...
	add_centered_blocks(lines, marks, {
		...
		{ " v0.2.0 ", "NvimeHeaderBlockSecondary" },
	}, width)
	add_centered_blocks(lines, marks, {
		{ "press ", "NvimeMuted" },
		{ " ? ", "NvimeKey" },
...
local function session_primary_line(session, kind, index)
	...
	local suffix =
		string.format("%s  %s  %s", provider, session_mode(session, kind), ui.relative_time(session.updated_at))
...
local function add_dashboard_rows(
	lines,
	row_to_session,
	row_to_kind,
	number_to_row,
...
local function add_dashboard_help(lines, section_rows, meta_rows)
	...
	meta_rows[#meta_rows + 1] = #lines
	lines[#lines + 1] = "  R              rename the session under cursor"
	meta_rows[#meta_rows + 1] = #lines
	...
	meta_rows[#meta_rows + 1] = #lines
	lines[#lines + 1] =
		"  Ask/Edit rows are scoped to a file range and keep their native provider session when possible."
	meta_rows[#meta_rows + 1] = #lines
end
...
local function render(bufnr)
	...
	elseif mode then
		...
		if #sessions == 0 then
			...
			meta_rows[#meta_rows + 1] = #lines
			lines[#lines + 1] =
				"  Select code and press visual <leader>nq or visual <leader>ne, or start from the current function above."
			meta_rows[#meta_rows + 1] = #lines
		else
			lines[#lines + 1] = "Selection Discussions"
			section_rows[#section_rows + 1] = #lines
...
local function rename_selected()
	...
	if not session_id then
		vim.notify("No nvime session under cursor", vim.log.levels.INFO)
		return
	end
	local kind = panel.row_to_kind and panel.row_to_kind[row] or (is_chat_mode() and "chat" or "selection")
	local current = ""
	...
	vim.ui.input({ prompt = "Rename: ", default = current }, function(input)
		if input == nil then
			return
		end
		...
		local ok = false
		if kind == "chat" then
			ok = chat.rename_session(session_id, title)
		else
			ok = selection.rename_session(session_id, title)
		end
		if ok then
			M.open({ mode = requested_mode() })
		end
