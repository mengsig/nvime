	...
	with_writable(bufnr, function()
		...
		if not start or start < 1 or start > count then
			...
			if fresh then
				...
				vim.api.nvim_buf_set_lines(bufnr, 0, count, false, {
					"",
					"",
...
function M.focus_input(opts)
	...
	if opts.preserve_cursor then
		...
		if cur_ok and cursor[1] == lnum and cursor[2] >= prefix_col then
...
function M.delete_sessions(ids)
	...
	for _, session in ipairs(ensure_sessions()) do
		if remove[tonumber(session.id)] then
			...
			if
				state.last_session
				and state.last_session.kind == "selection"
				and state.last_session.id == session.id
			then
