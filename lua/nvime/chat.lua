...
local function clear_session_process(session, handle)
  ...
  if not handle or session.process == handle then
    session.process = nil
    state.running["chat:" .. tostring(session.id)] = nil
  end
end

local function remember_session_process(session, handle)
  if not session or not handle then
    return
  end
  session.process = handle
  state.running["chat:" .. tostring(session.id)] = {
    kind = "chat",
    id = session.id,
    handle = handle,
  }
...
function M.attach_process(session_id, handle)
  local session = session_id and get_session(session_id) or active_session()
  remember_session_process(session, handle)
end
...
function M.cancel_active()
  return M.cancel_session(state.chat and state.chat.active_session_id)
end
...
function M.submit(text, opts)
  ...
  handle = agents.run({
    ...
    on_handle = function(agent_handle)
      handle = agent_handle
      remember_session_process(session, agent_handle)
    end,
